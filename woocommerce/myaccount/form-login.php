...
<?php if ('yes' === get_option('woocommerce_enable_myaccount_registration')): ?>
	...
	<div class="u-columns col2-set grid grid-cols-1 lg:grid-cols-2 gap-8 my-12 font-roboto" id="customer_login">
		<div class="u-column1 col-1">
		<?php endif; ?>
		<div class="bg-gray-50 rounded-none p-8">
			<h2 class="uppercase text-left text-xl md:text-3xl font-semibold tracking-wide mb-4">
				<?php esc_html_e('Login', 'woocommerce'); ?>
			</h2>
			<form class="woocommerce-form woocommerce-form-nonegin login" method="post">
				...
				<div class="form-row pt-4">
					<div class="flex justify-between items-center mb-4">
						<label
							class="woocommerce-form__label woocommerce-form__label-for-checkbox woocommerce-form-nonegin__rememberme">
							<input class="woocommerce-form__input woocommerce-form__input-checkbox " name="rememberme" type="checkbox"
								id="rememberme" value="forever" /> <span class="uppercase text-xs">
								<?php esc_html_e('Remember me', 'woocommerce'); ?>
							</span>
						</label>
						<p class="woocommerce-nonestPassword lost_password">
							<a class="underline hover:text-secondary transition-all duration-200 uppercase text-xs"
								href="<?php echo esc_url(wp_lostpassword_url()); ?>"><?php esc_html_e('Lost your password?', 'woocommerce'); ?></a>
						</p>
					</div>
					<?php wp_nonce_field('woocommerce-nonegin', 'woocommerce-nonegin-nonce'); ?>
					<button type="submit"
						class="woocommerce-button button woocommerce-form-nonegin__submit btn btn-primary flex justify-center"
		...
		<div class="u-column2 col-2">
			<div class="bg-white rounded-none p-8">
				<h2 class="uppercase text-left text-xl md:text-3xl font-semibold tracking-wide mb-4">
					<?php esc_html_e('Register', 'woocommerce'); ?>
				</h2>
				<form method="post" class="woocommerce-form woocommerce-form-register register font-roboto" <?php do_action('woocommerce_register_form_tag'); ?>>
					<?php do_action('woocommerce_register_form_start'); ?>
					<?php if ('no' === get_option('woocommerce_registration_generate_username')): ?>
						<p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
							<label class="uppercase text-xs font-semibold tracking-wide" for="reg_username">
					...
					<?php endif; ?>
					<p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
						<label class="uppercase text-xs font-semibold tracking-wide" for="reg_email">
					...
					<div class="my-4">
						<?php if ('no' === get_option('woocommerce_registration_generate_password')): ?>
							<p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
								<label class="uppercase text-xs font-semibold tracking-wide" for="reg_password">
									<?php esc_html_e('Password', 'woocommerce'); ?>&nbsp;<span class="required">*</span>
								</label>
								<input type="password"
									class="woocommerce-Input woocommerce-Input--text input-text w-full px-4 py-2 uppercase tracking-wide border text-xs rounded-none h-10"
