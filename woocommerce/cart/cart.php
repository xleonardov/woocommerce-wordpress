...
<div class="grid grid-cols-1 lg:grid-cols-[1fr_minmax(0,374px)] gap-4 py-8 tracking-wide">
	<div class="">
		<form class="woocommerce-cart-form w-full grid gap-4" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
					...
					<?php
						foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
							...
							if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
								...
								<div class="grid border-b pb-4 last:border-none last:pb-0 grid-cart-products gap-y-4 lg:grid-cols-[28px_160px_2fr_1fr_1fr_1fr] items-center woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
			...
			<div class="bg-white rounded-lg p-4">
				<?php if ( wc_coupons_enabled() ) { ?>
					<div class="coupon">
						<label for="coupon_code" class="block mb-4 uppercase text-sm font-semibold tracking-wide"><?php esc_html_e( 'Coupon', 'woocommerce' ); ?></label> 
						<div class="grid gap-4 grid-cols-[auto_144px]">
							<input type="text" name="coupon_code" class="input-text w-full px-4 py-2 uppercase tracking-wide border text-xs rounded-lg" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Coupon code', 'woocommerce' ); ?>" /> 
							<button type="submit" class="button bg-primary hover:bg-gray-700 transition-all duration-200 w-36 py-3 h-full flex justify-center items-center text-white uppercase text-xs tracking-wide rounded-lg" name="apply_coupon" value="<?php esc_attr_e( 'Apply coupon', 'woocommerce' ); ?>"><?php esc_attr_e( 'Apply coupon', 'woocommerce' ); ?></button>
						</div>
