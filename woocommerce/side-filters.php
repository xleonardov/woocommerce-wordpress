...
?>
<div class="bg-white rounded-lg overflow-y-auto grid content-start gap-4 max-h-[3008px] h-full" id="filters-box">
  <div class="overflow-auto">
		...
		if(count($new_args) > 0){
			foreach( wc_get_products($query_args) as $product ){
				foreach( $product->get_attributes() as $tax => $attribute ){
					if(in_array($tax, $toIgnore)) continue;
			...
			foreach($data as $attribute){
				foreach($query as $row){
					if($attribute["slug"] !== $row->attr_slug) continue;
					$term = new \stdClass();
				...
				<div class="mb-4 p-4">
					<ul>
						...
						foreach($new_args as $key=>$arg){ 
							...
							?>
							<li class="flex mb-2">
								<div class="clear-filter flex flex-row text-white bg-secondary py-1 px-2 rounded-full items-center space-x-1 cursor-pointer " data-value="<?=$arg?>" data-attribute="<?= $data['pa_'.$key]["slug"] ?>" data-taxonomy="<?= isset($args['category']->slug) ?  $args['category']->slug :''?>">
									<div class="pointer-events-none"><?= $icons->get_icon('AiOutlineCloseCircle') ?> </div>
					...
					<button class="clear-all-filters text-sm text-secondary" data-taxonomy="<?= isset($args['category']->slug) ?  $args['category']->slug :''?>">Limpar tudo (<?= count($new_args) ?>)</button>
				</div>
			<?php
			}
		foreach($data as $key=> $attribute){
			?>
			<div class="mb-4 p-4">
				<h4 class="font-bold uppercase mb-4 border-b"><?= $attribute["name"] ?></h4>
				<ul class="filters-box overflow-hidden transition-all duration-200 ease-in-out <?=count($attribute["terms"]) > 5 ? 'max-h-[160px]': ''?>">
					<?php foreach($attribute["terms"] as $term){ ?>
						<li class="flex items-center space-x-2 mb-2">
							<div class="relative w-6 h-6">
								<input type="checkbox" name="sidefilter" id="<?= $attribute["slug"].'_'.$term->slug?>" value="<?=$term->slug?>" data-attribute="<?= $attribute["slug"] ?>" data-taxonomy="<?=property_exists($args['category'], 'slug') ?  $args['category']->slug :''?>" class="appearance-none border border-gray-400 rounded-lg cursor-pointer w-6 h-6 checked:bg-secondary filter-checkbox" 
								<?php
