  ...
  <div
    ...
    <form class="grid grid-rows-[auto_1fr_auto] h-screen font-roboto" id="filters-form">
      <input type="hidden" name="taxonomy"
        value="<?= property_exists($args['category'], 'slug') ? $args['category']->slug : '' ?>" />
            ...
            <div class="flex gap-1 w-full flex-wrap">
              <?php foreach ($terms as $term):
                if ($type === "color"):
                  ...
                  ?>
                  <div>
                    <input type="checkbox" class="hidden f-checkbox" name="<?= $key ?>" id="<?= $key . '_' . $term->slug ?>"
                      value="<?= $term->slug ?>" data-attribute="<?= $key ?>"
                      data-taxonomy="<?= property_exists($args['category'], 'slug') ? $args['category']->slug : '' ?>" <?php
